...
public class APIWorkFlowSteps {
    ...
    @Given("a request is prepared for to retrieve the created employee")
    public void a_request_is_prepared_for_to_retrieve_the_created_employee() {
        request = given().header(apiConstants.Header_Content_type, apiConstants.Content_type)
                .header(apiConstants.Header_Authorization, GenerateTokenSteps.token)
    ...
    public void the_retrieved_employee_id_matches_the_globally_stored_employee_id(String employeeIDfromResponse) {
        //  response.then().assertThat().body(employee_id,equalTo(employeeIDfromResponse));
        String tempEmpID = response.jsonPath().getString(employeeIDfromResponse);
        Assert.assertEquals(employee_id, tempEmpID);

    }

    @Then("the retrieved data at {string} matches the data used to create an employee with employee with employee ID {string}")
    public void the_retrieved_data_at_matches_the_data_used_to_create_an_employee_with_employee_with_employee_id(
